...
public interface Extractable<T extends Tuple> {
    ...
    static <T1,T2> PartialFunction<Tuple2<T1,T2>,Void> _case(BiPredicate<T1,T2> p, BiConsumer<T1,T2> c) {
    ...
    static <T1,T2> PartialFunction<Tuple2<T1,T2>,Void> _otherwise(BiConsumer<T1,T2> c) {
    ...
    static <T1,T2,T> PartialFunction<Tuple2<T1,T2>,T> _case(BiPredicate<T1,T2> p, BiFunction<T1,T2,T> f) {
        return PartialFunction.fromPredicateAndFunction(m -> m.extract().map(t -> t.map((t1,t2) -> p.test(t1,t2))).orElse(false), t -> f.apply(t._1(), t._2()));
    }

    static <T1,T2,T> PartialFunction<Tuple2<T1,T2>,T> _otherwise(BiFunction<T1,T2,T> f) {
        return PartialFunction.fromFunction(t -> f.apply(t._1(), t._2()));
    }
}
